...
while ( have_posts() ) : the_post();
    ...
    $goal     = get_post_meta( get_the_ID(), '_nmm_project_goal',     true );
    $partners = get_post_meta( get_the_ID(), '_nmm_project_partners', true );
    $location = get_post_meta( get_the_ID(), '_nmm_project_location', true );
    $ext_link = get_post_meta( get_the_ID(), '_nmm_project_link',     true );
    $img_url  = nmm_get_post_thumbnail_url( get_the_ID(), 'nmm-hero' );

    $status_colors = [
        'Active' => '#27ae60', 'Ongoing' => '#27ae60',
        'Completed' => '#3498db', 'Planned' => '#f39c12', 'On Hold' => '#e74c3c',
    ];
        ...
        <div style="display:grid; grid-template-columns:1fr 300px; gap:3rem; align-items:start;">
            ...
            <article>
                ...
                <div style="display:grid; grid-template-columns:repeat(auto-fit,minmax(140px,1fr)); gap:1rem; margin-bottom:2.5rem;">
                    <?php
                    $meta = [
                        [ 'fas fa-tag',          'Status',   $status ],
                        [ 'fas fa-map-marker-alt','Location', $location ?: '—' ],
                        [ 'fas fa-calendar-check','Started',  $start ? date( 'M Y', strtotime( $start ) ) : '—' ],
                        [ 'fas fa-calendar-times','End Date', $end ? date( 'M Y', strtotime( $end ) ) : __( 'Ongoing', 'nas-medical-mission' ) ],
                    ];
                    if ( $goal ) $meta[] = [ 'fas fa-bullseye', 'Goal', $goal ];
                    foreach ( $meta as $m ) :
                    ?>
                        <div style="background:var(--color-surface); border:1px solid var(--color-border); border-radius:var(--radius-md); padding:1rem; text-align:center;">
                            ...
                            <span style="font-size:.9rem; font-weight:600; color:<?php echo ( $m[1] === 'Status' ? esc_attr( $status_color ) : 'var(--color-charcoal)' ); ?>; line-height:1.3;"><?php echo esc_html( $m[2] ); ?></span>
                        </div>
                    <?php endforeach; ?>
                </div>
                ...
                <!-- Partners -->
                <?php if ( $partners ) : ?>
                    <div style="background:var(--color-primary-pale); border:1px solid rgba(110,55,139,.15); border-radius:var(--radius-md); padding:1rem 1.25rem; margin-bottom:2rem; display:flex; align-items:center; gap:.75rem;">
                        <i class="fas fa-handshake" style="color:var(--color-primary); flex-shrink:0;"></i>
                        <span style="font-size:.9rem; color:var(--color-slate);">
                            <strong><?php esc_html_e( 'Partners / Funders:', 'nas-medical-mission' ); ?></strong>
                ...
                <?php else : ?>
                    <figure style="margin-bottom:2rem; border-radius:var(--radius-xl); overflow:hidden;">
                        <img src="<?php echo esc_url( $img_url ); ?>" alt="<?php echo esc_attr( get_the_title() ); ?>" style="width:100%;max-height:480px;object-fit:cover;" loading="eager">
                    </figure>
                <?php endif; ?>
                ...
                <?php if ( $ext_link ) : ?>
                    <div style="margin-top:2rem;">
                        <a href="<?php echo esc_url( $ext_link ); ?>" target="_blank" rel="noopener" class="btn btn-outline">
                            <i class="fas fa-external-link-alt"></i>
                            <?php esc_html_e( 'View Project Website', 'nas-medical-mission' ); ?>
                        </a>
                    </div>
                <?php endif; ?>
            ...
            <aside style="position:sticky; top:100px; display:flex; flex-direction:column; gap:1.5rem;">
                <div style="background:var(--color-primary); border-radius:var(--radius-lg); padding:2rem; color:white; text-align:center;">
                    ...
                    <h4 style="color:white; margin-bottom:.5rem;"><?php esc_html_e( 'Support This Project', 'nas-medical-mission' ); ?></h4>
                    <p style="font-size:.88rem; color:rgba(255,255,255,.8); margin-bottom:1.25rem;"><?php esc_html_e( 'Your donation directly funds this and other NAS Medical Mission initiatives.', 'nas-medical-mission' ); ?></p>
                ...
                <a href="<?php echo esc_url( home_url( '/volunteer/' ) ); ?>" class="btn btn-outline" style="justify-content:center;">
                    <i class="fas fa-hands-helping"></i> <?php esc_html_e( 'Volunteer With Us', 'nas-medical-mission' ); ?>
                </a>
                <a href="<?php echo esc_url( home_url( '/contact/' ) ); ?>" class="btn btn-outline" style="justify-content:center;">
                    <i class="fas fa-envelope"></i> <?php esc_html_e( 'Partner on This Project', 'nas-medical-mission' ); ?>
                </a>

                <!-- Other projects -->
                <?php
                $others = new WP_Query( [ 'post_type' => 'nmm_project', 'posts_per_page' => 4, 'post__not_in' => [ get_the_ID() ] ] );
                if ( $others->have_posts() ) :
                ?>
                    <div style="background:var(--color-surface); border-radius:var(--radius-lg); padding:1.5rem;">
                        <h4 style="font-size:.82rem; text-transform:uppercase; letter-spacing:.1em; color:var(--color-muted); margin-bottom:1rem;"><?php esc_html_e( 'Other Projects', 'nas-medical-mission' ); ?></h4>
